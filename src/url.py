...
def build_dict():
    ...
    unavailableSchedules = defaultdict(list)
    # skip quarters and weeks that are empty after the first iteration to prefvent repetition
    for option in cfg.options("OPTIONS"):
        if option == "c":
            print("c")
        for quarter in range(1, 3):
            for week in range(1, 47):
                # ex skip till kw1 week 45
                # {kw: [1,2,3,4]}
                if not((quarter in unavailableSchedules) & (week in unavailableSchedules[quarter])):
                    ...
                    while True:
                        ...
                        if resp[1] != 200:
                            unavailableSchedules[quarter].append(week)
                            break
                        else:
